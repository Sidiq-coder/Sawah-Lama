...
export default function WilayahInfo() {
  ...
  const mapData = data?.wilayahMap
  const hasMapGeometry = mapData?.geojson?.features?.length
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="container-section space-y-8 py-10">
              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    {item.value ? <p className="text-lg font-semibold text-brand-700">{item.value}</p> : null}
        ...
        {!isLoading ? (
          hasMapGeometry ? (
            <WilayahMap data={mapData} height="26rem" />
          ) : (
            <div className="rounded-3xl bg-slate-50 p-8 text-sm text-slate-500">
              Peta wilayah belum diunggah. Admin dapat menambahkan batas wilayah melalui dashboard.
            </div>
          )
        ) : null}
      </div>
